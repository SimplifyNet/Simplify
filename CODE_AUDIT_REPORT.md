...
### C1. Исключения в callback таймера роняют процесс
...
### C2. `throw;` внутри worker-задачи при отсутствии подписки на `OnException`
...
### H1. `CommonEqualityComparer`: `GetHashCode` несовместим с `Equals` + NRE
...
### H2. Утечка/блокировка scope для basic-job
...
### H3. XML-парсинг без hardening (защита от XXE)
...
### H4. Потеря исключений при `ParallelWhenAll` публикации событий
...
| # | Файл | Проблема |
|---|------|----------|
| M1 | `Simplify.System/WeakSingleton.cs:27-37` | Lock-free `TryGetTarget` гонится с `SetTarget` — `WeakReference<T>` не гарантирует потокобезопасность. |
| M2 | `Simplify.Log/Logger.cs` (Write/WriteToFile) | Log injection: CR/LF из сообщения не экранируются → подделка строк лога. |
| M3 | `Simplify.Log/Logger.cs:205-215` | TOCTOU при ротации: `Exists`→`Length`→`Delete`→`Append` без межпроцессной блокировки; ротация через `Delete` уничтожает лог. |
| M4 | `Simplify.Log/Logger.cs` / `Simplify.IO/FileHelper.cs:124` | `Assembly.GetCallingAssembly().Location` пуст в single-file/AOT → файлы уходят в CWD. Лучше `AppContext.BaseDirectory`. |
| M5 | `Simplify.FluentNHibernate/SessionFactoryBuilderBase.cs:34` | Публичное свойство `ConnectionString` отдаёт строку с паролем. |
| M6 | `Simplify.FluentNHibernate/ConfigurationExtensions.cs` (Oracle/MySQL/PG) | Проверка null-пароля есть только для MsSql; остальные провайдеры молча строят connection без пароля. |
| M7 | `Simplify.FluentNHibernate/Interceptors/SqlStatementInterceptor.cs:25-40` | При `ShowSql` весь SQL пишется в `Console`/`Trace` — утечка query-shape/литералов. |
| M8 | `Simplify.DI.Provider.Microsoft.Extensions.DependencyInjection/MicrosoftDependencyInjectionDIProvider.cs:60` | Дефолтный lifetime `Singleton` против `PerLifetimeScope` у остальных → captive dependencies. |
| M9 | `Simplify.DI.Wcf/SimplifyInstanceProvider.cs` | На ошибке scope диспоузится, но остаётся прицеплен к `InstanceContext` → double-dispose в `ReleaseInstance`. |
| M10 | `Simplify.Templates/Template.cs:109` | Подстановка переменных без экранирования; значение с `{Other}` подменяется на след. проходе — инъекция в HTML/email. |
| M11 | `Simplify.Windows.Forms/ControlsValidator.cs`, `TextBoxesValidator.cs` | Повторный `EnableValidation()` подписывает хендлеры повторно (`+=` без `-=`) → утечка/мульти-валидация. |
| M12 | `Simplify.System/Extensions/BytesExtensions.cs:15` | Нечётная длина массива молча теряет последний байт; зависит от endianness. |
| M13 | `Simplify.Scheduler/SchedulerJobsHandler.cs` (`_jobs`) | `_jobs` (обычный `List<>`) читается/мутируется из разных потоков (Ctrl+C handler, Dispose, Stop) без синхронизации. |
...
4. **M2/M3/M4** (Logger) и **M8/M9** (DI lifetime) — следующая волна.
